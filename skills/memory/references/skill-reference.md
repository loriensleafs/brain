...
Unified memory search across Brain and Forgetful.

### Description

Agent-facing skill script that wraps MemoryRouter.psm1. Provides unified memory search with Brain-first routing and optional Forgetful augmentation per ADR-037.
...
Search only Brain (lexical/file-based). Faster and requires no network access.
...
```json
{
  ...
  "Results": [...],
  "Diagnostic": {
    "Brain": { "Available": true, "Path": "notes/" },
...
----                    ------    ----- -------
powershell-arrays       Brain     1.0   PowerShell arrays need @() for...
...
```json
{
  "Query": "string",
  "Count": 0,
  "Source": "Unified|Brain|Forgetful",
  "Results": [
    {
      "Name": "memory-name",
      "Source": "Brain|Forgetful",
      "Score": 1.0,
      "Path": "/path/to/memory",
  ...
  "Results": [
    {
      ...
      "Source": "Brain",
      "Score": 1.0,
      "Path": "notes/git-hooks-pre-commit.md",
...
Name                    Source Score Preview
----                    ------ ----- -------
powershell-array-handling Brain 1.0   PowerShell arrays need @() f...
powershell-arrays        Brain 1.0   Common array operations incl...
```
...
| Brain MCP | Yes | Lexical memory search |
...
Store new memories to Brain with optional Forgetful sync.

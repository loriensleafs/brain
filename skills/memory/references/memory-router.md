...
The Memory Router (`.claude/skills/memory/scripts/MemoryRouter.psm1`) provides unified memory search across Brain (lexical) and Forgetful (semantic) memory systems with Brain-first routing.
...
The Memory Router always queries Brain first (canonical source), then optionally augments with Forgetful semantic results.

**Rationale**: Brain travels with the Git repository, ensuring cross-platform availability. Forgetful provides enhanced semantic search but requires a running service.

### Result Augmentation

Forgetful results enhance but never replace Brain results.

**Merge Strategy**:

1. Query Brain (always)
2. Query Forgetful (if available and not LexicalOnly)
3. Deduplicate by SHA-256 content hash
4. Return Brain results + unique Forgetful matches

### Availability Detection

The router automatically detects Forgetful availability using a cached TCP health check (30s TTL, 500ms timeout).

**Failure Mode**: Gracefully degrades to Brain-only if Forgetful unavailable.
...
# Unified search (Brain + Forgetful if available)
...
Force Brain-only search (skip Forgetful):
...
| LexicalOnly | Switch | No | - | Force Brain-only search (always available) |

**Returns**: Array of `PSCustomObject` with:

- `Name`: Memory name
- `Content`: Full memory content
- `Source`: "Brain" or "Forgetful"
- `Score`: Relevance score (percentage for Brain, similarity for Forgetful)
- `Path`: File path (Brain only)
...
#     Source: "Brain",
#     Score: 66.67,
#     Path: "notes//powershell-array-handling.md",
#     Hash: "a3b5c7..."
#   },
#   ...
# ]
...
if (Test-ForgetfulAvailable) {
    ...
    Write-Host "Forgetful is unavailable, using Brain-only"
}
```
...
Performs lexical search across Brain memory files.
...
Merges and deduplicates results from Brain and Forgetful.

**Algorithm**:

1. Create hash set from Brain results (SHA-256 of content)
2. Add all Brain results to merged set
...
**Brain Priority**: Brain results appear first and are the canonical version on content collision.
...
|-----------|---------|-------|
| Brain search | ~530ms | O(n) file scan + keyword match |
...
| **Total (Brain-only)** | ~530ms | Baseline, no network |
| **Total (augmented)** | ~700ms | Brain + Forgetful + merge |
...
|------------|----------|----------|
| Brain | Local file I/O | No network exposure |
...
# Returns Brain results only, no error
```
...
1. **Always use Search-Memory**: Don't call Brain/Forgetful MCP directly
...
1. **Test both modes**: Verify Brain-only and augmented searches

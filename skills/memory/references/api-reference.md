...
- **LexicalOnly** (Switch, Optional): Force Brain-only search (always available)

**Returns**: `PSCustomObject[]` with properties:

- `Name` (String): Memory name
- `Content` (String): Full memory content
- `Source` (String): "Brain" or "Forgetful"
- `Score` (Double): Relevance score (Brain: percentage, Forgetful: similarity)
- `Path` (String): File path (Brain only, nullable)
...
- `Brain` (Hashtable):
  - `Available` (Boolean): Whether Brain path exists
  - `Path` (String): Configured Brain notes path
- `Forgetful` (Hashtable):
...
- `Configuration` (Hashtable):
  - `BrainPath` (String): Brain notes directory
...
Write-Host "Brain: $($status.Brain.Available), Forgetful: $($status.Forgetful.Available)"
```
...
@{
    ...
    chosen    = "Brain-first"            # String: Chosen option
...
|----------|----------------|------------|
| Search-Memory (Brain-only) | 530ms | O(n) file scan |

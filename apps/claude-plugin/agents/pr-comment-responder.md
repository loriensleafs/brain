...
- **Brain MCP memory tools**: PR review patterns, bot behaviors
...
**Note**: Statistics are sourced from the `pr-comment-responder-skills` memory (use `mcp__plugin_brain_brain__read_note` with `identifier="pr-comment-responder-skills"`) and should be updated there after each PR review session.
...
mcp__plugin_brain_brain__read_note(identifier="pr-comment-responder-skills")
```
...
for reviewer in ALL_REVIEWERS:
    if reviewer == "cursor[bot]":
        mcp__plugin_brain_brain__read_note(identifier="cursor-bot-review-patterns")
    elif reviewer == "copilot-pull-request-reviewer":
        mcp__plugin_brain_brain__read_note(identifier="copilot-pr-review-patterns")
...
current = mcp__plugin_brain_brain__read_note(identifier="pr-comment-responder-skills")
...
mcp__plugin_brain_brain__edit_note(
    ...
    find_text="| cursor[bot] |",  # Find the row start
    content=f"| cursor[bot] | {new_total_comments} | {new_actionable} | **{new_rate}%** |"
)
...
mcp__plugin_brain_brain__edit_note(
    ...
    section="Per-PR Breakdown",
    content=new_pr_section
)
```
...
Use Brain MCP memory tools directly for cross-session context. Memory is critical for PR comment handling - reviewers have predictable patterns.

**At start (MANDATORY):**

```python
mcp__plugin_brain_brain__search(query="PR review patterns bot behaviors reviewer preferences")
```
...
```python
mcp__plugin_brain_brain__edit_note(
    identifier="PR-Pattern-[Category]",
    operation="append",

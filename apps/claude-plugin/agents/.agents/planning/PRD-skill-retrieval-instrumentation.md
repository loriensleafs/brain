...
**Detection Method**: Search for `mcp__plugin_brain_brain__read_note` calls where `memory_file_name` parameter matches pattern `skill-*` or `skills-*`
...
**Approach**: Parse existing `.agents/sessions/*.md` files to extract `mcp__plugin_brain_brain__read_note` calls.
...
Session logs must contain tool call sections showing `mcp__plugin_brain_brain__read_note` invocations. Example:

```xml
<invoke name="mcp__plugin_brain_brain__read_note">

...
if ($Operation -eq 'Delete') {
    ...
    $memories = mcp__plugin_brain_brain__search(, @{
        query = $memoryName
...
if (-not $Force) {
    $existing = mcp__plugin_brain_brain__search(, @{
        query = $memoryName
...
if ($existing.primary_memories.Count -gt 0) {
    ...
    mcp__plugin_brain_brain__edit_note(, ($memoryData + @{ memory_id = $forgetfulId }))
    Write-Verbose "Updated Forgetful memory $forgetfulId from Serena $Path"
} else {
    # Create new
    mcp__plugin_brain_brain__write_note(, $memoryData)
...
foreach ($file in $serenaMemories) {
    ...
    $forgetful = mcp__plugin_brain_brain__search(, @{
        query = $memoryName
...
# Check for orphaned Brain memories
$allBrainNotes = mcp__plugin_brain_brain__list_directory()
$serenaNames = @($serenaMemories | ForEach-Object { $_.BaseName })

foreach ($brainNote in $allBrainNotes) {
    if ($brainNote.title -notin $serenaNames) {
        $stats.Orphaned++
    }
}

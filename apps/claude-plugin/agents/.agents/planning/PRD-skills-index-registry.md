...
Agents currently discover skills by calling `mcp__plugin_brain_brain__list_directory`, which returns 100+ memory names, then calling `mcp__plugin_brain_brain__read_note` multiple times to find relevant skills. This is an O(n) linear search that scales poorly as skills grow (currently ~7 new skills per day).
...
**Rationale**: Placing the index in `.serena/memories/` allows agents to read it using `mcp__plugin_brain_brain__read_note` with `memory_file_name="skills-index"`, consistent with existing memory access patterns.
...
- `mcp__plugin_brain_brain__list_directory`: ~100ms
- `mcp__plugin_brain_brain__read_note` per file: ~50ms
- Total for 5 skills: ~350ms

**Target State**: O(1) lookup with index

- `mcp__plugin_brain_brain__read_note` for index: ~50ms
- Agent scans table in-memory: ~10ms
- `mcp__plugin_brain_brain__read_note` for specific skill: ~50ms
...
- Serena MCP memory system (`mcp__plugin_brain_brain__read_note`, `mcp__plugin_brain_brain__write_note`)

...
### Phase 1: Brain Initialization (BLOCKING)

The agent MUST complete Brain initialization before any other action. This is a **blocking gate**.

**Requirements:**

1. The agent MUST call `mcp__plugin_brain_brain__bootstrap_context` with the project path as the first tool call
2. The agent MUST load initial context by reading relevant notes via `mcp__plugin_brain_brain__read_note`
...
- Brain tools (read_note, write_note, edit_note, etc.) become functional

**Rationale:** Without Brain initialization, agents lack access to project notes, semantic knowledge graph, and historical context. This causes repeated mistakes and lost decisions.
...
2. The agent MUST read the `memory-index` note using `mcp__plugin_brain_brain__read_note` to identify task-relevant notes
3. The agent MUST read notes from `memory-index` that match the task keywords before modifying code or files
...
The `memory-index` maps task keywords to essential notes. Example workflow:

```text
1. Identify task keywords (e.g., "pr review", "powershell", "github cli")
2. Read memory-index, find matching rows
3. Load listed notes (e.g., skills-pr-review-index, skills-powershell-index)
4. Apply learned patterns from notes before proceeding
```

**Evidence**: 30% session efficiency loss observed when notes not loaded first (skill-init-003-memory-first-monitoring-gate).
...
**Rationale:** Agents are expert amnesiacs. Without reading HANDOFF.md, they will repeat completed work or contradict prior decisions. Note: HANDOFF.md is a read-only reference; do not modify it during sessions. Without loading relevant notes, agents repeat solved problems or miss established patterns.

### Phase 3: Import Shared Notes (RECOMMENDED)

The agent SHOULD import shared notes at session start.

**Requirements:**

1. The agent SHOULD run the import script (if available):

   ```bash
   pwsh .claude-mem/scripts/Import-ClaudeMemMemories.ps1
   ```

1. The agent SHOULD document import count in session log
1. The agent MAY skip if no note files present
...
**Rationale:** Shared note exports enable team collaboration, onboarding, and cross-session knowledge transfer. Script is idempotent with automatic duplicate prevention.
...
**Rationale:** Session 15 had 5+ skill violations despite documentation. Trust-based compliance fails; verification-based enforcement (like Brain init) has 100% compliance.
...
- Branch matches conventional naming patterns (feat/_, fix/_, docs/\*, etc.)
...
### Phase 0.5: Export Session Notes (RECOMMENDED)

The agent SHOULD export notes created during the session for sharing and version control.

**Requirements:**

1. The agent SHOULD export notes using session-specific naming:

   ```bash
   pwsh .claude-mem/scripts/Export-ClaudeMemMemories.ps1 -Query "[query]" -SessionNumber NNN -Topic "topic"
   ```

1. The agent MUST perform security review before committing (BLOCKING):
...
1. The agent MUST NOT commit exports containing sensitive data
1. The agent SHOULD document export file path in session log
1. The agent MAY skip export for sessions without significant note creation
...
**Rationale:** Export files contain plain text note data. Committing sensitive information to git creates permanent security risks. Security review is BLOCKING before commit.
...
4. The agent SHOULD update PROJECT-PLAN.md if tasks were completed
5. The agent MAY read `.agents/HANDOFF.md` for historical context (read-only reference)
...
1. The agent MUST wait for QA validation to complete
1. The agent MUST NOT commit feature code without QA validation
1. The agent MAY skip QA validation when:
...
1. The agent MUST NOT commit if branch mismatch detected (stop and investigate)
1. The agent MUST stage all changed files including `.agents/` files
1. The agent MUST commit with conventional commit message format
1. The agent SHOULD verify clean git status after commit
1. The agent MAY push to remote if appropriate
...
1. The agent MAY skip retrospective for trivial sessions (single file edits, documentation-only)
...
| MUST     | Commit all changes (including .brain/notes)                                                                                      | [ ]      | Commit SHA: **\_\_\_**                                               |
| MUST NOT | Update `.agents/HANDOFF.md` directly                                                                                             | [ ]      | HANDOFF.md unchanged                                                 |
| SHOULD   | Update PROJECT-PLAN.md                                                                                                           | [ ]      | Tasks checked off                                                    |
| SHOULD   | Invoke retrospective (significant sessions)                                                                                      | [ ]      | Doc: **\_\_\_**                                                      |

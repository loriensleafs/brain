...
**You are the Brain Orchestrator agent.** Your full definition lives in `agents/orchestrator.md`. This document describes the system you coordinate — the agent catalog, workflows, memory architecture, and quality gates.
...
| :------ | :------------ | :--- |
| **Orchestrator** (root level) | Delegate to **memory agent** via `Task(subagent_type="memory", ...)` | Always prefer the memory agent |
| **Subagents** (1 level deep) | Load the **memory skill** via `Skill(skill="brain:memory")` | MUST use skill directly — cannot delegate further |

When routing tasks to subagents that will need memory access, include in the prompt: "Load the memory skill before any memory operations."
...
**File**: `agents/orchestrator.md` — **This is your full agent definition. Read it.**
...
**File**: `agents/independent-thinker.md`
...
**File**: `agents/high-level-advisor.md`
...
**File**: `agents/pr-comment-responder.md`
...
**Reference**: These workflows are the canonical patterns defined in `agents/orchestrator.md`. The orchestrator coordinates all agent delegation using a one-level-deep pattern (orchestrator → subagent → back to orchestrator).
...
**Note**: This workflow integrates with Standard Flow. Orchestrator coordinates all delegation. See `agents/orchestrator.md` for delegation patterns.
...
mcp__plugin_brain_brain__edit_note
{
  ...
  "operation": "append",
  "content": "[Observation with source]"
}
...
mcp__plugin_brain_brain__write_note
{
  "title": "[Note-Title]",
  "category": "[Type]",
  "content": "[Note content with observations]"
}
```

Follow the memory skill for Brain MCP tool usage. See memory skill (`skills/memory/SKILL.md`)/load memory skill `Skill(skill="brain:memory")` for:

- Required entity type to folder mappings
...
**Specs folder naming**: `{ENTITY-NNN-topic}` is the canonical name of the parent entity. Use the parent's ALL CAPS prefix and kebab-case topic (e.g., `specs/ADR-015-auth-strategy/requirements/REQ-001-token-validation.md`).
...
```text
sessions/SESSION-2025-12-29-19-parallel-pr-300.md
sessions/SESSION-2025-12-29-20-parallel-pr-301.md
sessions/SESSION-2025-12-29-21-parallel-pr-302.md
```
...
3. Updates Brain memory with cross-session context
...
| `agent-prompts.md` | `agents/**/*.md` | Agent prompt patterns | 9 |
...
| implementer          | opus   | Balanced code generation |
| analyst              | opus   | Research efficiency      |
| architect            | opus   | Design analysis          |
| planner              | opus   | Planning speed           |
| critic               | opus   | Review efficiency        |
| qa                   | opus   | Test validation          |
| explainer            | opus   | Documentation            |
| task-generator       | opus   | Task decomposition       |
| high-level-advisor   | opus   | Strategic depth          |
| independent-thinker  | opus   | Deep analysis            |
| memory               | opus   | Simple operations        |
...
1. **Create Agent Prompt**: `agents/[agent-name].md`
...
1. **Create Steering File**: `steering/[domain].md`

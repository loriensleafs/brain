...
**You are the Brain Orchestrator agent.** Your full identity and execution protocol live in `agents/orchestrator.md`. This document provides operational instructions for the system you coordinate.

### Memory Delegation Hierarchy

| Context | Memory Method | Rule |
| :------ | :------------ | :--- |
| **You (orchestrator)** at root level | Delegate to **memory agent** via `Task(subagent_type="memory", ...)` | Always prefer the memory agent for memory operations |
| **Subagents** (already 1 level deep) | Load and follow the **memory skill** via `Skill(skill="brain:memory")` | Subagents CANNOT delegate further. They MUST use the memory skill directly for ALL memory/note operations |
...
- [ ] Read `agents/orchestrator.md` — your full agent definition
- [ ] Read `planning/enhancement-PROJECT-PLAN.md` for current project
- [ ] Identify your assigned phase and tasks
- [ ] Create session log using the memory skill: `SESSION-YYYY-MM-DD_NN-{topic}.md`

**Memory note**: When delegating to subagents, include in their prompt: "Load the memory skill (`Skill(skill='brain:memory')`) before any memory operations." Subagents cannot delegate to the memory agent.
...
| `planning/FEATURE-{NNN}-{topic}.md` | Master project plan | After task completion |
...
1. Read the full task description in `planning/FEATURE-{NNN}-{topic}.md`
...
1. ✅ Check off the task in `planning/FEATURE-{NNN}-{topic}.md`
...
| MUST | Update Brain memory (cross-session context) via `mcp__plugin_brain_brain__edit_note` | [x] | Memory write confirmed |
...
2. **Unclear what to do**: Re-read `planning/FEATURE-{NNN}-{topic}.md`

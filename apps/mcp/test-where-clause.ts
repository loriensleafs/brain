...
const sample = db.query('SELECT embedding FROM brain_embeddings LIMIT 1').get() as any;
...
try {
  ...
  if (test3.length > 0) {
    test3.forEach((r: any) => console.log(`    - ${r.chunk_id}: distance=${r.distance}`));
  }
} catch (e: any) {
  console.log(`  Error: ${e.message}`);
}

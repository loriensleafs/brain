...
const count = db.query('SELECT COUNT(*) as count FROM brain_embeddings').get() as any;
console.log(`Total embeddings in table: ${count.count}`);

// Get a sample embedding from the table
const sample = db.query('SELECT chunk_id, entity_id, chunk_index, embedding FROM brain_embeddings LIMIT 1').get() as any;
if (sample) {
  console.log(`\nSample embedding:

...
const DIRECTORY_MAP: Record<string, { folder: string; entityType: AgentEntityType }> = {
...
export function parseAgentFile(
  ...
  const entityType = detectEntityType(relativeDir, sourcePath, originalFrontmatter);
...
function isValidEntityType(type: string): boolean {
  const validTypes: AgentEntityType[] = [
    'session', 'decision', 'requirement', 'design', 'task',
    'analysis', 'feature', 'epic', 'critique', 'test-report',
    'security', 'retrospective', 'skill', 'note'
  ];
...
export function parseMarkdownTable(content: string): Map<string, string> {
  ...
  while ((match = tableRowPattern.exec(content)) !== null) {
    ...
    if (['Req', 'Step', 'Status', 'Evidence', 'Finding', 'Severity'].includes(key)) {
      continue;
    }

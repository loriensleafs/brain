...
describe("transformToBasicMemory", () => {
  ...
  it("generates context section", () => {
    ...
    sections.set(
      "Context",
      "This session focused on testing the migration pipeline.",
    );
  ...
  it("extracts tags from multiple sources", () => {
    ...
    expect(result.tags.some((t) => t.includes("complete") || t === "p0")).toBe(
      true,
    );
  });
  ...
  it("transforms ADR correctly", () => {
    ...
    expect(result.observations.some((o) => o.category === "decision")).toBe(
      true,
    );
    expect(result.observations.some((o) => o.category === "problem")).toBe(
      true,
    );
  });
  ...
  it("transforms requirement correctly", () => {
    ...
    expect(result.observations.some((o) => o.category === "requirement")).toBe(
      true,
    );
...
describe("validateTransformation", () => {
  ...
  it("warns on too few relations", () => {
    const note = {
      ...
      relations: [{ type: "relates_to" as const, target: "A" }],
      context: "Valid context.",
      fullContent: "...",
    };
...
describe("formatObservationsMarkdown", () => {
  ...
  it("handles observations without tags", () => {
    ...
    const observationLine = markdown
      .split("\n")

...
describe("generateObservations", () => {
  describe("session entity", () => {
    it("extracts objective observation", () => {
      ...
      sections.set(
        "Objective",
        "Implement the authentication flow for user login.",
      );
      ...
      expect(
        observations.some(
          (o) =>
            o.category === "fact" && o.content.includes("authentication flow"),
        ),
      ).toBe(true);
    });
...
`,
      ...
      expect(
        observations.some(
          (o) => o.category === "insight" && o.tags.includes("finding"),
        ),
      ).toBe(true);
    });
    ...
    it("extracts branch information from frontmatter", () => {
      ...
      expect(
        observations.some(
          (o) => o.content.includes("fix/auth-flow") && o.tags.includes("git"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("decision entity", () => {
    it("extracts decision statement", () => {
      ...
      sections.set(
        "Decision",
        "Use JWT tokens for authentication with 24-hour expiry.",
      );
      ...
      expect(
        observations.some(
          (o) => o.category === "decision" && o.content.includes("JWT tokens"),
        ),
      ).toBe(true);
    });
    ...
    it("extracts status from frontmatter", () => {
      ...
      expect(
        observations.some(
          (o) => o.content.includes("implemented") && o.tags.includes("status"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("requirement entity", () => {
    it("extracts requirement statement", () => {
      ...
      expect(
        observations.some(
          (o) => o.category === "requirement" && o.content.includes("OAuth2"),
        ),
      ).toBe(true);
    });
...
`,
      ...
      expect(
        observations.some((o) => o.tags.includes("acceptance-criteria")),
      ).toBe(true);
    });
  });
...
`,
      ...
      expect(
        observations.some(
          (o) => o.category === "problem" && o.tags.includes("security"),
        ),
      ).toBe(true);
    });
    ...
    it("extracts remediation as solution", () => {
      ...
      sections.set(
        "Remediation",
        "Replace bash parsing with PowerShell using validated functions.",
      );
      ...
      expect(
        observations.some(
          (o) => o.category === "solution" && o.content.includes("PowerShell"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("quality thresholds", () => {
    ...
    it("fills with metadata when content is sparse", () => {
      ...
      expect(
        observations.some((o) => o.content.includes("Sparse Document")),
      ).toBe(true);
    });
  });
});

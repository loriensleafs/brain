  ...
  describe("buildSessionEnrichment", () => {
    ...
    test("extracts agent history from orchestrator workflow", async () => {
      ...
      const invocation1: AgentInvocation = {
        ...
        output: { artifacts: [], summary: "Analysis complete", recommendations: [], blockers: [] },
        handoffFrom: null,
        handoffTo: "planner",
    ...
    test("limits agent history to maxAgentHistory", async () => {
      ...
      const agentTypes = [
        "analyst", "planner", "implementer", "qa", "architect",
        "critic", "security", "devops", "retrospective", "memory",
      ] as const;
      ...
      workflow.agentHistory = agentTypes.map((agent, i) => ({
        ...
        output: { artifacts: [], summary: `Summary ${i}`, recommendations: [], blockers: [] },
        handoffFrom: null,
        handoffTo: null,
        handoffReason: "",
      }));
            ...
            results: [
              {
                ...
                title: "OAuth Integration Feature",
                permalink: "features/oauth-integration",
                content: "Feature details with tasks: [[TASK-001]] and [[TASK-002]]",
              },
            ],
          }),
        }],

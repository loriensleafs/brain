...
func init() {
	...
	schemaPath := filepath.Join(packageRoot, "schemas", "tools", "bootstrap-context.schema.json")
	data, err := os.ReadFile(schemaPath)
	if err != nil {
		panic("failed to load bootstrap schema for tests: " + err.Error())
	}
	SetBootstrapSchemaData(data)
}

...
`
	result := validation.ValidateSlashCommandFromContent(content, "analyze.md")
...
# Content
`
	result := validation.ValidateSlashCommandFromContent(content, "test.md")
...
`},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := validation.ValidateSlashCommandFromContent(tt.content, "test.md")
...
func TestValidateSlashCommand_FileNotFound(t *testing.T) {
	result := validation.ValidateSlashCommand("/nonexistent/path/command.md")
...
`
	...
	result := validation.ValidateSlashCommand(filePath)
...
func TestValidateSlashCommandDirectory_EmptyDir(t *testing.T) {
	...
	results := validation.ValidateSlashCommandDirectory(tmpDir)
...
func TestValidateSlashCommandDirectory_NonexistentDir(t *testing.T) {
	results := validation.ValidateSlashCommandDirectory("/nonexistent/path")
...
`
	...
	results := validation.ValidateSlashCommandDirectoryRecursive(tmpDir)
...
func TestValidateSlashCommandFiles_EmptyList(t *testing.T) {
	results := validation.ValidateSlashCommandFiles([]string{})
...
`
	...
	results := validation.ValidateSlashCommandFiles(filePaths)
...
func TestCountBlockingViolations(t *testing.T) {
	results := []validation.SlashCommandValidationResult{
		{ValidationResult: validation.ValidationResult{Valid: true}},
		{ValidationResult: validation.ValidationResult{Valid: false}},
		{ValidationResult: validation.ValidationResult{Valid: false}},
		{ValidationResult: validation.ValidationResult{Valid: true}},
	}

	count := validation.CountBlockingViolations(results)
...
func TestCountWarnings(t *testing.T) {
	...
	count := validation.CountWarnings(results)
...
func TestValidateSlashCommandFromContent_EmptyContent(t *testing.T) {
	result := validation.ValidateSlashCommandFromContent("", "test.md")
...
`
	result := validation.ValidateSlashCommandFromContent(content, "generate-test-data.md")
...
`
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validation.ValidateSlashCommandFromContent(content, "analyze.md")
	}
}
...
`
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validation.ValidateSlashCommandDirectory(tmpDir)
	}
}

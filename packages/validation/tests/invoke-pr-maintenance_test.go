...
func TestGetBotAuthorInfo_AgentControlled(t *testing.T) {
	config := validation.DefaultPRMaintenanceConfig()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := validation.GetBotAuthorInfo(tt.author, config)

			if !result.IsBot {
				t.Errorf("Expected %s to be identified as bot", tt.author)
			}
			if result.Category != validation.BotCategoryAgentControlled {
...
func TestGetBotAuthorInfo_MentionTriggered(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Category != validation.BotCategoryMentionTriggered {
...
func TestGetBotAuthorInfo_ReviewBot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Category != validation.BotCategoryReviewBot {
...
func TestGetBotAuthorInfo_Human(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Category != validation.BotCategoryHuman {
...
func TestIsBotReviewer_AgentControlledReviewer(t *testing.T) {
	config := validation.DefaultPRMaintenanceConfig()
	requests := []validation.ReviewRequest{
	...
	if !validation.IsBotReviewer(requests, config) {
...
func TestPRHasConflicts_Conflicting(t *testing.T) {
	pr := validation.PullRequest{
		Mergeable: validation.MergeableConflicting,
	}

	if !validation.PRHasConflicts(pr) {
...
func TestPRHasConflicts_Mergeable(t *testing.T) {
	pr := validation.PullRequest{
		Mergeable: validation.MergeableMergeable,
	}
...
func TestPRHasConflicts_Unknown(t *testing.T) {
	pr := validation.PullRequest{
		Mergeable: validation.MergeableUnknown,
	}
...
func TestPRHasFailingChecks_NoCommits(t *testing.T) {
	...
	if validation.PRHasFailingChecks(pr) {
...
func TestPRHasFailingChecks_NoRollup(t *testing.T) {
	pr := validation.PullRequest{}
	pr.Commits.Nodes = []validation.PRCommit{{}}
...
func TestPRHasFailingChecks_OverallFailure(t *testing.T) {
	...
	pr.Commits.Nodes = []validation.PRCommit{
		{
			Commit: struct {
				StatusCheckRollup *validation.StatusCheckRollup `json:"statusCheckRollup,omitempty"`
			}{
				StatusCheckRollup: &validation.StatusCheckRollup{
					State: validation.CheckStateFailure,
				},
			},
		},
	}
	...
	pr.Commits.Nodes = []validation.PRCommit{
		{
			Commit: struct {
				...
				StatusCheckRollup: &validation.StatusCheckRollup{
					State: validation.CheckStateError,
				},
			},
		},
	}
...
func TestPRHasFailingChecks_ContextFailure(t *testing.T) {
	pr := validation.PullRequest{}
	rollup := &validation.StatusCheckRollup{
		State: validation.CheckStateSuccess,
	}
	rollup.Contexts.Nodes = []validation.StatusCheckContext{
		{Name: "test", Conclusion: validation.ConclusionFailure},
	}
	pr.Commits.Nodes = []validation.PRCommit{
		{
			Commit: struct {
...
func TestPRHasFailingChecks_AllSuccess(t *testing.T) {
	...
	rollup.Contexts.Nodes = []validation.StatusCheckContext{
		{Name: "test1", Conclusion: validation.ConclusionSuccess},
		{Name: "test2", Conclusion: validation.ConclusionSuccess},
	}
	pr.Commits.Nodes = []validation.PRCommit{
		{
			Commit: struct {
...
func TestGetDerivativePRs_HasDerivatives(t *testing.T) {
	...
	prs := []validation.PullRequest{
		...
		{Number: 2, BaseRefName: "feature-a", HeadRefName: "feature-b", Author: validation.PRAuthor{Login: "dev"}, Title: "Sub-PR"},
	}
...
func TestGetPRsWithPendingDerivatives_NoParents(t *testing.T) {
	...
	derivatives := []validation.DerivativePR{
		{Number: 2, TargetBranch: "feature-b"},
	}

	result := validation.GetPRsWithPendingDerivatives(prs, derivatives)
...
func TestClassifyPR_AgentControlledWithChangesRequested(t *testing.T) {
	...
	pr := validation.PullRequest{
		Number:         1,
		Title:          "Test PR",
		Author:         validation.PRAuthor{Login: "rjmurillo-bot"},
		ReviewDecision: validation.ReviewChangesRequested,
		Mergeable:      validation.MergeableMergeable,
	}

	actionItem, blockedItem := validation.ClassifyPR(pr, config)
	...
	if actionItem.Category != validation.BotCategoryAgentControlled {
		t.Errorf("Expected category 'agent-controlled', got '%s'", actionItem.Category)
	}
	if actionItem.Reason != validation.ReasonChangesRequested {
...
func TestClassifyPR_AgentControlledWithConflicts(t *testing.T) {
	...
	if actionItem.Reason != validation.ReasonHasConflicts {
...
func TestClassifyPR_MentionTriggeredWithIssues(t *testing.T) {
	...
	pr := validation.PullRequest{
		Number:         3,
		Title:          "Copilot PR",
		Author:         validation.PRAuthor{Login: "copilot-swe-agent"},
		ReviewDecision: validation.ReviewChangesRequested,
	}
	...
	if actionItem.Category != validation.BotCategoryMentionTriggered {
...
func TestClassifyPR_HumanWithChangesRequested(t *testing.T) {
	...
	pr := validation.PullRequest{
		Number:         4,
		Title:          "Human PR",
		Author:         validation.PRAuthor{Login: "developer"},
		ReviewDecision: validation.ReviewChangesRequested,
	}
...
func TestClassifyPR_HumanNoIssues(t *testing.T) {
	...
	pr := validation.PullRequest{
		...
		Title:          "Clean PR",
		Author:         validation.PRAuthor{Login: "developer"},
		ReviewDecision: validation.ReviewApproved,
		Mergeable:      validation.MergeableMergeable,
	}
...
func TestClassifyPR_BotReviewerTriggersAction(t *testing.T) {
	...
	pr.ReviewRequests.Nodes = []validation.ReviewRequest{
		{RequestedReviewer: struct {
			Login string `json:"login,omitempty"`
			Name  string `json:"name,omitempty"`
		}{Login: "rjmurillo-bot"}},
...
func TestAnalyzePRs_EmptyList(t *testing.T) {
	...
	result := validation.AnalyzePRs(prs, config)
...
func TestAnalyzePRs_MixedPRs(t *testing.T) {
	...
	prs := []validation.PullRequest{
		...
		},
		{
			Number:         3,
			Title:          "Clean PR",
			Author:         validation.PRAuthor{Login: "other-dev"},
			ReviewDecision: validation.ReviewApproved,
			BaseRefName:    "main",
		},
	}
...
func TestAnalyzePRs_WithDerivatives(t *testing.T) {
	...
	for _, item := range result.ActionRequired {
		if item.Number == 1 && item.Reason == validation.ReasonPendingDerivs {
			found = true
			break
		}
	}
...
func TestSortActionRequired_ConflictsFirst(t *testing.T) {
	...
	sorted := validation.SortActionRequired(items)
...
func TestFormatMaintenanceOutput(t *testing.T) {
	result := validation.PRMaintenanceResult{
		TotalPRs: 5,
		ActionRequired: []validation.PRActionItem{
			{Number: 1, HasConflicts: true},
			{Number: 2, HasConflicts: false},
		},
		Blocked: []validation.PRActionItem{
			{Number: 3},
		},
		DerivativePRs: []validation.DerivativePR{
			{Number: 4},
		},
	}

	output := validation.FormatMaintenanceOutput(result)
...
func TestPRMaintenanceOutput_ToJSON(t *testing.T) {
	output := validation.PRMaintenanceOutput{
		PRs: []validation.PRActionItem{
			{Number: 1, Category: validation.BotCategoryAgentControlled, Reason: validation.ReasonChangesRequested},
		},
		Summary: validation.PRMaintenanceSummary{
			Total:          5,
			ActionRequired: 1,
			Blocked:        0,
			Derivatives:    0,
...
func TestCheckRateLimitSafe_Safe(t *testing.T) {
	result := validation.CheckRateLimitSafe(100, 50)
...
func TestCheckRateLimitSafe_CoreTooLow(t *testing.T) {
	result := validation.CheckRateLimitSafe(50, 100)
...
func TestCheckRateLimitSafe_GraphQLTooLow(t *testing.T) {
	result := validation.CheckRateLimitSafe(200, 25)
...
func TestCheckRateLimitSafe_BothTooLow(t *testing.T) {
	result := validation.CheckRateLimitSafe(50, 25)
...
func TestParsePRsFromJSON_Valid(t *testing.T) {
	...
	prs, err := validation.ParsePRsFromJSON(jsonData)
	if err != nil {
...
func TestParsePRsFromJSON_Invalid(t *testing.T) {
	...
	_, err := validation.ParsePRsFromJSON(jsonData)
	if err == nil {
...
func TestParseRateLimitFromJSON_Valid(t *testing.T) {
	...
	result, err := validation.ParseRateLimitFromJSON(jsonData)
	if err != nil {
...
func TestParseRateLimitFromJSON_Invalid(t *testing.T) {
	...
	_, err := validation.ParseRateLimitFromJSON(jsonData)
	if err == nil {
		t.Error("Expected error for invalid JSON")
	}
}
...
func TestClassifyPR_FailingChecksOnly(t *testing.T) {
	...
	if actionItem.Reason != validation.ReasonHasFailingChecks {
...
func TestClassifyPR_MentionTriggeredPrioritizesChangesRequested(t *testing.T) {
	...
	actionItem, _ := validation.ClassifyPR(pr, config)

...
`
	...
	result := validation.ValidatePRDescription(description, filesInPR)
...
func TestValidatePRDescriptionWithConfig_CustomExtensions(t *testing.T) {
	config := validation.DefaultPRDescriptionConfig()
	...
	result := validation.ValidatePRDescriptionWithConfig(config)
...
`
	...
	result := validation.ValidatePRDescriptionSections(description, requiredSections)
...
`
	...
	result := validation.ValidatePRDescriptionSections(description, nil)
...
`
	...
	result := validation.ValidatePRChecklist(description)
...
`
	...
	result := validation.ValidatePRDescriptionFull(description, filesInPR, requiredSections)
...
`
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validation.ValidatePRDescription(description, filesInPR)
	}
}
...
`
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validation.ValidatePRDescriptionFull(description, filesInPR, requiredSections)
	}
}
...
`
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validation.ValidatePRDescription(description, []string{})
	}
}

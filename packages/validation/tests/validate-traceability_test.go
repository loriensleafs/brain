...
`
	...
	spec := validation.ParseYAMLFrontmatter(filePath)
...
func TestParseYAMLFrontmatter_NonexistentFile(t *testing.T) {
	spec := validation.ParseYAMLFrontmatter("/nonexistent/path/file.md")
...
func TestLoadAllSpecs_EmptyDirectory(t *testing.T) {
	...
	specs := validation.LoadAllSpecs(specsDir)
...
func TestTestTraceabilityRules_Rule1_ForwardTraceability(t *testing.T) {
	...
	requirements := map[string]*validation.SpecFrontmatter{
		"REQ-001": {ID: "REQ-001", Status: "draft"},
		"REQ-002": {ID: "REQ-002", Status: "draft"}, // Orphaned
	}
	designs := map[string]*validation.SpecFrontmatter{
		"DESIGN-001": {ID: "DESIGN-001", Status: "draft", Related: []string{"REQ-001"}},
	}
	tasks := map[string]*validation.SpecFrontmatter{}

	specs := &validation.SpecCollection{
		Requirements: requirements,
		Designs:      designs,
		Tasks:        tasks,
		All:          make(map[string]*validation.SpecFrontmatter),
	}

	result := validation.TestTraceabilityRules(specs)
...
func TestValidateTraceability_NonexistentPath(t *testing.T) {
	result := validation.ValidateTraceability("/nonexistent/path/specs", false)
...
func TestValidateTraceability_EmptySpecs(t *testing.T) {
	...
	result := validation.ValidateTraceability(specsDir, false)
...
`
	...
	result := validation.ValidateTraceability(specsDir, true)
...
func TestValidateTraceabilityFromContent_Valid(t *testing.T) {
	...
	result := validation.ValidateTraceabilityFromContent(requirements, designs, tasks, false)
...
func TestFormatTraceabilityResults_JSON(t *testing.T) {
	result := validation.TraceabilityValidationResult{
		ValidationResult: validation.ValidationResult{
			Valid:   true,
			Message: "All traceability checks passed",
		},
		Stats: validation.TraceabilityStats{
			Requirements: 1,
			Designs:      1,
			Tasks:        1,
			ValidChains:  1,
	...
	output := validation.FormatTraceabilityResults(result, "json")
...
func TestFormatTraceabilityResults_Markdown(t *testing.T) {
	result := validation.TraceabilityValidationResult{
		...
		Errors: []validation.TraceabilityIssue{
			{Rule: "Rule 4", Source: "TASK-001", Message: "Broken reference"},
		},
		Warnings: []validation.TraceabilityIssue{
			{Rule: "Rule 1", Source: "REQ-002", Message: "Orphaned requirement"},
		},
	}

	output := validation.FormatTraceabilityResults(result, "markdown")
...
func TestFormatTraceabilityResults_Console(t *testing.T) {
	...
	output := validation.FormatTraceabilityResults(result, "console")
...
func TestFormatTraceabilityResults_DefaultToConsole(t *testing.T) {
	result := validation.TraceabilityValidationResult{
		Stats: validation.TraceabilityStats{Requirements: 1},
	}

	output := validation.FormatTraceabilityResults(result, "unknown-format")
...
func TestValidateTraceability_TaskWithMultipleDesignRefs(t *testing.T) {
	...
	result := validation.ValidateTraceabilityFromContent(requirements, designs, tasks, true)

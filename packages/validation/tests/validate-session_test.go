...
func TestValidateStopReadiness_NilState(t *testing.T) {
	result := validation.ValidateStopReadiness(nil)
	...
	var stateCheck *validation.Check
...
func TestValidateStopReadiness_NoBlockingOpsCheck(t *testing.T) {
	...
	var blockCheck *validation.Check
	for i := range result.Checks {
...
func TestValidateSession_NilState(t *testing.T) {
	result := validation.ValidateSession(nil)
...
func TestCheckQASkipEligibility_NoFiles(t *testing.T) {
	result := validation.CheckQASkipEligibility([]string{})

	if !result.Eligible {
		t.Error("Expected QA skip to be eligible for empty file list")
	}

	if result.SkipType != validation.QASkipDocsOnly {
...
func TestCheckQASkipEligibility_DocsOnly(t *testing.T) {
	...
	result := validation.CheckQASkipEligibility(files)
...
func TestCheckQASkipEligibility_InvestigationOnly(t *testing.T) {
	...
	if result.SkipType != validation.QASkipInvestigationOnly {
...
func TestIsDocsOnly_Empty(t *testing.T) {
	if !validation.IsDocsOnly([]string{}) {
...
func TestIsDocsOnly_AllMarkdown(t *testing.T) {
	...
	if !validation.IsDocsOnly(files) {
...
func TestGetImplementationFiles_FiltersAuditArtifacts(t *testing.T) {
	...
	result := validation.GetImplementationFiles(files)
...
func TestGetImplementationFiles_Empty(t *testing.T) {
	result := validation.GetImplementationFiles([]string{})
	if len(result) != 0 {
...
func TestValidateMemoryEvidence_ValidEvidence(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := validation.ValidateMemoryEvidence(tt.evidence)
...
func TestParseChecklistTable(t *testing.T) {
	...
	rows := validation.ParseChecklistTable(tableLines)
...
func TestDetectTemplateDrift_NoDrift(t *testing.T) {
	protocolRows := []validation.ChecklistRow{
		{Requirement: "MUST", Step: "Initialize Brain"},
		{Requirement: "MUST", Step: "Load context"},
	}

	sessionRows := []validation.ChecklistRow{
		{Requirement: "MUST", Step: "Initialize Brain"},
		{Requirement: "MUST", Step: "Load context"},
	}

	result := validation.DetectTemplateDrift(sessionRows, protocolRows)
...
func TestNormalizeStep(t *testing.T) {
	...
	for _, tt := range tests {
		result := validation.NormalizeStep(tt.input)
...
func TestValidateSessionLogPath_Valid(t *testing.T) {
	...
	result := validation.ValidateSessionLogPath(sessionPath, tmpDir)
...
`
	...
	result := validation.ExtractStartingCommit(content)
...
func TestValidateQARow_SkipInvestigationOnly(t *testing.T) {
	...
	eligibility := validation.QASkipResult{
		Eligible: true,
		SkipType: validation.QASkipInvestigationOnly,
	}

	result := validation.ValidateQARow(row, eligibility)
...
func TestValidateQARow_SkipDocsOnly(t *testing.T) {
	...
	eligibility := validation.QASkipResult{
		Eligible: true,
		SkipType: validation.QASkipDocsOnly,
	}
...
func TestValidateCommitSHAEvidence_Valid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := validation.ValidateCommitSHAEvidence(tt.evidence)
...
func TestCheckInvestigationOnlyEligibility_AllAllowed(t *testing.T) {
	...
	result := validation.CheckInvestigationOnlyEligibility(files)

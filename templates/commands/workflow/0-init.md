...
2. **Read session context** - Restore cross-session context from Brain memory
3. **Query relevant memories** - Retrieve session history and learned patterns via `mcp__plugin_brain_brain__search`
4. **Create session log** - Initialize a new session log entry in Brain `sessions/` folder
5. **Declare current branch** - Record the active Git branch and working state

## MCP Integration

Maps to Session State MCP (ADR-011). Fallback: execute steps manually using Brain MCP tools and file reads.
...
| Session context loaded | BLOCKING | Content present in context |
| Session log created | REQUIRED | Note exists with correct template |

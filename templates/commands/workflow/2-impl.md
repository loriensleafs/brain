...
Maps to Agent Orchestration MCP (ADR-013): `invoke_agent()`, `start_parallel_execution()`, `aggregate_parallel_results()`, `resolve_conflict()`, `track_handoff()`. Fallback: sequential `Agent()` invocations with manual context passing.
...
```text
/2-impl Add the authentication methods per plan

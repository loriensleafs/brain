...
allowed-tools: Bash(git checkout -b:*), Bash(git switch -c:*), Bash(git add:*), Bash(git status:*), Bash(git push:*), Bash(git commit:*), Bash(gh pr create:*), Bash(git diff:*), Bash(git branch:*)
...
5. Create a pull request using `gh`:
   ...
   ```bash
   gh pr create --title "<conventional commit title>" --body-file /tmp/PR-123-BODY.md
   ```

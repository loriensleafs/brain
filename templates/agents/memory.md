...
- **Brain MCP search**: Unified semantic/keyword search
  - Brain memory search: Search with automatic semantic/keyword fallback
...
- **Brain memory tools**: Search, read, write, and edit notes
  - Brain memory list directory: List notes in folders
  - Brain memory read note: Read specific note by identifier
  - Brain memory write note: Create new note with folder, title, content
  - Brain memory edit note: Update existing note (append, prepend, find_replace, replace_section)
  - Brain memory delete note: Remove obsolete note
  - Brain memory bootstrap context: Initialize project context at session start
...
**Important**: The memory agent does not manage paths directly. Brain memory tools automatically:
...
- `features/` - Feature artifacts per [[ADR-001-feature-workflow]] (FEAT/REQ/DESIGN/TASK)
- `roadmap/` - Strategic direction, epics, prioritization
- `planning/` - PRDs from explainer agent
...
- `- requires [[FEAT-001-user-authentication]]`
...
| ------------------- | ------------------------------- | --------------------------------------------------------------------- |
| Project Handoff     | `governance/handoff`            | `Brain memory read`            |
| Project Constraints | `governance/constraints`        | `Brain memory read`        |
| Naming Conventions  | `governance/naming-conventions` | `Brain memory read` |

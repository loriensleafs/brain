...
- **github skill**: `github operations` - unified GitHub operations

## GitHub Skill Integration

**MANDATORY**: Use the unified github skill at `github operations` for all GitHub operations. The skill provides tested, validated scripts with proper error handling, pagination, and security validation.
...
```powershell
# All scripts are in github operationsscripts/
# From repo root, invoke with pwsh:
pwsh github operationsscripts/pr/Get-PRContext.ps1 -PullRequest 50

# For global installs, use $HOME:
pwsh $HOME/github operationsscripts/pr/Get-PRContext.ps1 -PullRequest 50
```

See `github operationsSKILL.md` for full documentation.
...
**Note**: Statistics are sourced from the `pr-comment-responder-skills` memory (use Brain memory read note with `identifier="pr-comment-responder-skills"`) and should be updated there after each PR review session.
...
if [ -d "$SESSION_DIR" ]; then
  ...
  CURRENT_COMMENTS=$(pwsh github operationsscripts/pr/Get-PRReviewComments.ps1 -PullRequest [number] -IncludeIssueComments | jq '.TotalComments')
...
pwsh github operationsscripts/pr/Get-PRContext.ps1 -PullRequest [number] -IncludeChangedFiles
...
pwsh github operationsscripts/pr/Get-PRReviewers.ps1 -PullRequest [number]

# Exclude bots from enumeration
pwsh github operationsscripts/pr/Get-PRReviewers.ps1 -PullRequest [number] -ExcludeBots
```
...
pwsh github operationsscripts/pr/Get-PRReviewComments.ps1 -PullRequest [number] -IncludeIssueComments
...
$comments = pwsh -NoProfile github operationsscripts/pr/Get-PRReviewComments.ps1 -PullRequest [number] -IncludeIssueComments | ConvertFrom-Json
$ids = $comments.Comments | ForEach-Object { $_.id }

# Batch acknowledge - single process, all comments
$result = pwsh -NoProfile github operationsscripts/reactions/Add-CommentReaction.ps1 -CommentId $ids -Reaction "eyes" | ConvertFrom-Json
...
pwsh github operationsscripts/reactions/Add-CommentReaction.ps1 -CommentId [comment_id] -Reaction "eyes"

# Issue comment reaction
pwsh github operationsscripts/reactions/Add-CommentReaction.ps1 -CommentId [comment_id] -CommentType "issue" -Reaction "eyes"
```
...
pwsh github operationsscripts/pr/Post-PRCommentReply.ps1 -PullRequest [number] -CommentId [comment_id] -Body "[response]"

# For multi-line responses, use -BodyFile to avoid escaping issues
pwsh github operationsscripts/pr/Post-PRCommentReply.ps1 -PullRequest [number] -CommentId [comment_id] -BodyFile reply.md

# Post new top-level PR comment (no CommentId = issue comment)
pwsh github operationsscripts/pr/Post-PRCommentReply.ps1 -PullRequest [number] -Body "[response]"
```
...
```powershell
# Using github skill (PREFERRED)
pwsh github operationsscripts/pr/Post-PRCommentReply.ps1 -PullRequest [pull_number] -CommentId [comment_id] -Body "Fixed in [commit_hash].`n`n[Brief summary of change]"

# Or use a file for multi-line content
pwsh github operationsscripts/pr/Post-PRCommentReply.ps1 -PullRequest [pull_number] -CommentId [comment_id] -BodyFile resolution.md
```
...
pwsh github operationsscripts/pr/Resolve-PRReviewThread.ps1 -PullRequest [number] -All

# Or resolve a single thread by ID
pwsh github operationsscripts/pr/Resolve-PRReviewThread.ps1 -ThreadId "PRRT_kwDOQoWRls5m7ln8"
```
...
NEW_COMMENTS=$(pwsh github operationsscripts/pr/Get-PRReviewComments.ps1 -PullRequest [number] -IncludeIssueComments | jq '.TotalComments')
...
$checks = pwsh -NoProfile github operationsscripts/pr/Get-PRChecks.ps1 -PullRequest [number] -Wait -TimeoutSeconds 300 | ConvertFrom-Json
...
$checks = pwsh -NoProfile github operationsscripts/pr/Get-PRChecks.ps1 -PullRequest [number] | ConvertFrom-Json
...
new_pr_section = f"""#### PR #{PR_NUMBER} ({date})
...
Use Brain memory tools for cross-session context. Memory is critical for PR comment handling - reviewers have predictable patterns.

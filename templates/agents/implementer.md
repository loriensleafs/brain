...
**Keywords**: Code, SOLID, TypeScript, Bun, Tests, Production, Execution, Quality, Patterns, Commits, Build, Coverage, Refactor, Performance, Principles, DRY, Encapsulation, Unit-tests, Validation, Ship
...
mcp__plugin_brain_brain__search({ query: "[memory-from-table-above]", limit: 10 })
# Or read directly:
...
| ASI05 | Code Execution | `eval()`, `new Function()` with variables |
| ASI06 | Memory Poisoning | Unvalidated memory imports |
| ASI07 | Inter-Agent Comms | Agent delegation without validation |
...
- **Bash**: `bun build`, `bun test`, `biome check`, git commands
...
Read complete plans from planning artifacts, validate alignment with project objectives, and execute code changes step-by-step while maintaining quality standards.
...
|----------|-----------|----------|
| **Authentication/Authorization** | Login flows, tokens, permissions | JWT handling, session management, middleware guards |
| **Data Protection** | Encryption, hashing, PII | `Bun.password`, `Bun.CryptoHasher`, GDPR data |
...
| **File System** | File operations | Path construction, `Bun.file()`, `Bun.write()`, temp files |
| **Environment/Config** | Secret management | `.env` files, config with credentials, key storage |
| **Execution** | Dynamic code/commands | `eval()`, `new Function()`, SQL queries |
...
2. **Document in Implementation Notes**: Add to implementation notes
...
Save as Brain memory note in `analysis/` folder with title `ANALYSIS-NNN-impact-code-[feature]`:
...
```typescript
function printReport(customerId: string): void {
    if (!isValid(customerId)) throw new Error("Invalid customer ID");
    const employees = getEmployees(customerId);
...
- Rows (Recipient, Format, etc.) become Strategy interfaces
- Columns (Email, SMS, Push) become Concrete implementations via Abstract Factory
...
class EmailFactory implements NotificationFactory {
    createFormatter(): HtmlFormatter { return new HtmlFormatter(); }
    createSender(): SmtpSender { return new SmtpSender(); }
    createRateLimiter(): HourlyLimiter { return new HourlyLimiter(100); }
}
```
...
```text
mcp__plugin_brain_brain__search({ query: "implementation patterns [component/feature]", limit: 10 })
```
...
mcp__plugin_brain_brain__write_note({
  title: "ANALYSIS-NNN-pattern-implementation-[topic]",
  folder: "analysis",
...
- Minimize allocations. Use `Bun.ArrayBufferSink` for building large buffers incrementally
- Use `Bun.file()` and `Bun.write()` over `fs` equivalents (10x faster for many operations)
- Use `Bun.serve()` for HTTP, `bun:sqlite` for local data (native bindings, no N-API overhead)
- Use streaming APIs (`ReadableStream`, `Response` with stream body) for large payloads
- Use `Bun.hash()` for non-cryptographic hashing (faster than crypto)
- Optimize for branch prediction

### Testing

- Provide `bun:test` tests for ALL code
...
4. **No stderr suppression**: Never use `2>/dev/null` without
...
```markdown
- [ ] Read plan from planning artifacts
...
**Rationale**: Large PRs delay review and increase merge risk.
...
bun build && bun test && biome check .
```
...
- [ ] Build succeeds (`bun build` exits 0)

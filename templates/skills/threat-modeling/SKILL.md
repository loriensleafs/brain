...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/generate_threat_matrix.ts \
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/generate_mitigation_roadmap.ts \
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/validate_threat_model.ts \
...
|--------|---------|-------|
| `generate_threat_matrix.ts` | Create structured threat matrix | `bun run scripts/generate_threat_matrix.ts --scope "Name" --output path.md` |
| `generate_mitigation_roadmap.ts` | Create prioritized roadmap | `bun run scripts/generate_mitigation_roadmap.ts --input threats.md --output roadmap.md` |
| `validate_threat_model.ts` | Validate model completeness | `bun run scripts/validate_threat_model.ts <model.md>` |
...
Query Brain memory for prior threat models:

```text
mcp__plugin____brain__search({ query: "threat model authentication", folder: "security" })
```
...
mcp__plugin____brain__write_note({
  title: "SEC-001 Auth Service Threat Model Summary",
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/validate_threat_model.ts <model.md>
```
...
| Custom validation rules | Modify `validate_threat_model.ts` |

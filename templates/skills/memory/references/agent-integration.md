...
│  │  Search-Memory │  │   Module       │  │  Brain MCP     │ │
│  │     .ps1       │  │   Import       │  │  Tools         │ │
│  └───────┬────────┘  └───────┬────────┘  └───────┬────────┘ │
└──────────┼───────────────────┼───────────────────┼──────────┘
           │                   │                   │
...
```python
# Brain MCP Tools
mcp__plugin_brain_brain__search(query="powershell arrays")
mcp__plugin_brain_brain__read_note(identifier="powershell-arrays")
mcp__plugin_brain_brain__write_note(title="new-pattern", content="...")
mcp__plugin_brain_brain__edit_note(identifier="existing-note", operation="append", content="...")
mcp__plugin_brain_brain__build_context(topic="authentication patterns")
```
...
- Semantic search via build_context
...
mcp__plugin_brain_brain__read_note(identifier="usage-mandatory")
...
mcp__plugin_brain_brain__write_note(title="[relevant-memory]", content="...")

# 4. Commit changes
git add .agents/memory/ notes/
...
3. **Check Availability**: Handle Brain MCP unavailability gracefully
...
```powershell
try {
    $results = Search-Memory -Query "[topic]"
}
catch {
    Write-Warning "Brain MCP unavailable, using lexical search"
...
| Search-Memory (semantic) | 500-1000ms | Via Brain build_context |

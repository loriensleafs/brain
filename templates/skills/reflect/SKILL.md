...
1. **Check Brain memories**: Search for observation notes via `mcp__plugin____brain__search({ query: "{skill-name} observations" })`
2. **Infer from context**: Identify which skill(s) were used in the conversation
3. **Create if needed**: If missing, propose `{skill-name}-observations` note in the `skills/` folder

**Storage**: Brain MCP (canonical) via `mcp__plugin____brain__write_note` to the `skills/` folder
...
1. **Read existing memory** (if exists) via `mcp__plugin____brain__read_note`
...
5. **Write to Brain**: `mcp__plugin____brain__write_note` or `mcp__plugin____brain__edit_note`

**Storage**: Brain MCP (canonical) - write observation notes to the `skills/` folder with the naming pattern `SKILL-{NNN}-{skill-name}-observations.md`
...
3. **Add citations to Brain note** as observations with `[fact]` category
...
|   |       |       |   |
|   |       |       |   +--> User says Y -> Update Brain memory
|   |       |       |   |
...
Skill memories integrate with the Brain memory system:

```text
# Search skill sidecar learnings
mcp__plugin____brain__search({ query: "github observations constraints" })

# Read specific skill sidecar
mcp__plugin____brain__read_note({ identifier: "SKILL-001-github-observations" })
```
...
| Memory updated | Note written to Brain via MCP |
...
### Agent Sidecar Naming: `{skill-name}-observations`

**Decision**: Skill memories follow the Brain note pattern (e.g., `SKILL-001-github-observations.md`).

**Rationale**:

- Reuses the Brain entity type convention instead of inventing a parallel structure
- Keeps `{domain}-{description}` format while making "skill-sidecar" explicit
- Discovery via Brain search prevents orphaned learnings
- Single Canonical Store: Brain MCP writes to the knowledge graph, eliminating dual-governance ambiguity
...
- Document any manual edits in the session log before completion.
...
3. **Session log fixer** - after reflection, ensure the session log captures the learning summary via `session-log-fixer`.

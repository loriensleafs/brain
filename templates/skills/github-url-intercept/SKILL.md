...
```bash
# PR URL → Use this
bun run .claude/skills/github/scripts/pr/get_pr_context.ts --pull-request {n} --owner {owner} --repo {repo}

# Issue URL → Use this
bun run .claude/skills/github/scripts/issue/get_issue_context.ts --issue {n} --owner {owner} --repo {repo}
...
Action:
  ...
  2. Route: bun run .claude/skills/github/scripts/pr/get_pr_checks.ts --pull-request 735 --owner rjmurillo --repo ai-agents
```
...
Action:
  bun run .claude/skills/github/scripts/pr/get_pr_context.ts --pull-request 123 --owner owner --repo repo
```
...
```bash
bun run .claude/skills/github-url-intercept/scripts/test_url_routing.ts <github-url>
```

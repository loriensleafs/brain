---
name: session-end
description: Validate and complete session logs before commit. Auto-populates session end evidence (commit SHA, lint results, memory updates) and runs validation. Use when finishing a session, before committing, or when session validation fails.
license: MIT
agents:
  - implementer
  - devops
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/complete_session_log.ts
```
...
4. Validate with validate_session_json
5. Report pass/fail with actionable next steps

### Preview Changes First

```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/complete_session_log.ts --dry-run
```
...
| `brainMemoryUpdated` | Check Brain memory updates | MUST |
...
| `validationPassed` | Run validate_session_json | MUST |

### What You Must Provide Manually

- Brain memory updates (write notes via Brain MCP before running)
...
- Updated Brain memories if applicable
...
bun run ${CLAUDE_SKILL_DIR}/scripts/complete_session_log.ts --session-path ".agents/sessions/2026-02-07-session-05.json"
...
```text
[TODO] Brain memory not updated - write notes via Brain MCP before completing
...
- [ ] Brain memory updated (or flagged)
...
--- Changes ---
  ...
  Confirmed Brain memory updated
...
|--------|---------|------------|
| [complete_session_log.ts](scripts/complete_session_log.ts) | Auto-populate and validate session end | 0=success, 1=validation failed |
...
- [new_session_log_json.ts](../session-init/scripts/new_session_log_json.ts) - Session creation script

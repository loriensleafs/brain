...
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target src/services/auth.ts

# Assess changed files only (CI mode)
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target . --changed-only --format json

# Full module assessment
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target src/services/ --format markdown --output quality-report.md
```
...
The skill runs automated assessment via `scripts/assess.ts`:

1. **Symbol Extraction**
   - Detect language
   - Use LSP tool (if available)
...
1. **Comparison (if historical data)**
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target <path> [options]
```
...
```json
{
  ...
  "ignore": ["**/generated/**", "**/*.pb.py", "**/migrations/**"]
}
```
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target src/models/user.ts
```

Output:

```markdown
# Code Quality Assessment: src/models/user.ts
...
```bash
# In CI pipeline
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target . --changed-only --format json --output quality.json
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target src/ --format markdown --output reports/quality.md
```
...
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target src/ --format json | \
...
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target affected-files.txt > baseline.md

# After implementing ADR
bun run ${CLAUDE_SKILL_DIR}/scripts/assess.ts --target affected-files.txt > post-implementation.md
...
- [Calibration Examples](references/calibration-examples.md) - Scoring examples for team calibration
- [Refactoring Patterns](references/refactoring-patterns.md) - Remediation patterns for low scores
...
LSP integration improves accuracy when available.
...
- **Sergeant (assess.ts)**: Orchestrates workflow, delegates to specialists
- **Privates (score\_\*.ts)**: Focus on one quality each, report back

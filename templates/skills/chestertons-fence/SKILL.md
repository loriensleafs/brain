...
|-------|--------|-------------|
| File path or ADR number | Investigation report | `analysis/` folder in Brain memory |
...
bun run ${CLAUDE_SKILL_DIR}/scripts/investigate.ts --target path/to/file.py --change "remove unused validation"

# Investigate an ADR
bun run ${CLAUDE_SKILL_DIR}/scripts/investigate.ts --target .agents/architecture/ADR-005.md --change "allow bash scripts"

# Output as JSON (for automation)
bun run ${CLAUDE_SKILL_DIR}/scripts/investigate.ts --target path/to/file.py --change "description" --format json
```

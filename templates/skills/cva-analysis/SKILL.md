...
4. **Optional**: Export to Mermaid diagram using `bun run ${CLAUDE_SKILL_DIR}/scripts/export-cva-matrix.ts`
...
1. **Check for multidimensional variability**:
...
1. **Route to decision-critic** (if abstraction recommended):
...
1. **Route to architect agent** (for ADR creation):

   ```bash
   # Hand off ADR stub to architect agent
   Agent(subagent_type="architect", prompt="Create ADR from CVA analysis stub")
   ```

1. **Document reassessment triggers**:
...
```python
Agent(subagent_type="architect", prompt="Create ADR from CVA analysis at cva-payment-processing.md")
```
...
```python
Agent(subagent_type="independent-thinker", prompt="Challenge whether payment abstraction is justified given only 2 current methods")
```
...
- [ ] Validation script passes: `bun run ${CLAUDE_SKILL_DIR}/scripts/validate-cva-matrix.ts [file]`
- [ ] ADR stub created with decision rationale
...
See `references/matrix-building-examples.md` for complete TypeScript examples:

- Middleware Pipeline (commonality: process request, variability: authentication, logging, compression)
...
- [Matrix Building Examples](references/matrix-building-examples.md) - TypeScript examples with complete CVA workflows

## Scripts

### validate-cva-matrix.ts
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/export-cva-matrix.ts \
  --input cva-matrix.md \
  --format mermaid \
...
1. **Matrix Formats**: Add export formats beyond Markdown
   - Use `bun run ${CLAUDE_SKILL_DIR}/scripts/export-cva-matrix.ts --format <json|plantuml|csv>`

1. **Multidimensional Analysis**: Handle complex cases with 3+ axes

1. **AI Integration**: Automated pattern suggestion

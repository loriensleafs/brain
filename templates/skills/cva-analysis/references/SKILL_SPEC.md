...
<skill_specification version="1.0">
  ...
  <architecture>
    ...
    <phases>
      ...
      <phase id="5">
        ...
        <process>
          <step order="1">Run validation script: `bun run scripts/validate-cva-matrix.ts`</step>
  ...
  <evolution_analysis>
    ...
    <extension_points>
      ...
      <extension_point>
        ...
        <mechanism>Create `scripts/export-cva-matrix.ts` with format parameter</mechanism>
      </extension_point>
      <extension_point>
    ...
    <dependencies>
      ...
      <dependency type="external">
        <name>Bun + TypeScript for validation script</name>
        <stability>stable</stability>
        <fallback>Manual validation using checklist if Bun unavailable</fallback>
      </dependency>
    </dependencies>
  ...
  <success_criteria>
    <criterion id="SC1" type="functional" priority="must">
      ...
      <verification_method>Run `bun run scripts/validate-cva-matrix.ts [matrix-file.md]`, check exit code 0</verification_method>
    </criterion>
  ...
  <verification_protocol>
    ...
    <script_validation>
      <check type="script_exists">validate-cva-matrix.ts exists in scripts/</check>
  ...
  <generation_instructions>
    ...
    <reference_docs>
      ...
      <doc>references/matrix-building-examples.md - TypeScript examples with complete CVA workflows</doc>
    </reference_docs>

    <scripts_needed>
      <script type="validation">scripts/validate-cva-matrix.ts - Check matrix completeness (≥2 rows, ≥2 cols, cells filled, patterns suggested)</script>
      <script type="generation">scripts/generate-cva-template.ts - Create empty matrix template from user input</script>
      <script type="export">scripts/export-cva-matrix.ts - Convert Markdown table to Mermaid diagram</script>
    </scripts_needed>

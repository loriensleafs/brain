...
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts --target .

# Check only changed files (CI mode)
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts --git-staged

# Check specific files
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts src/models/user.cs src/services/auth.cs

# JSON output for automation
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts --target . --format json --output violations.json
```
...
Source: Analysis of closed PR reviewer patterns.
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts [options] [files...]
```
  ...
  - repo: local
    hooks:
      - id: style-enforcement
        name: Style Enforcement
        entry: bun run .claude/skills/style-enforcement/scripts/check_style.ts
        args: [--git-staged]
        language: system
...
jobs:
  style:
    ...
    steps:
      ...
      - name: Check style
        run: |
          bun run .claude/skills/style-enforcement/scripts/check_style.ts \
            --target . \
            --format sarif \
...
```bash
bun run .claude/skills/style-enforcement/scripts/check_style.ts --git-staged
```
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts src/models/User.cs
```
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts --target . --format json
```
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/check_style.ts --target . --format sarif --output results.sarif
```

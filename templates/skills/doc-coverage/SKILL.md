...
bun run ${CLAUDE_SKILL_DIR}/scripts/check_docs.ts --target .

# Check only changed files (CI mode)
bun run ${CLAUDE_SKILL_DIR}/scripts/check_docs.ts --git-staged

# Check specific files
bun run ${CLAUDE_SKILL_DIR}/scripts/check_docs.ts src/models/user.cs src/services/auth.py

# JSON output for automation
bun run ${CLAUDE_SKILL_DIR}/scripts/check_docs.ts --target . --format json --output coverage.json

# Set minimum threshold
bun run ${CLAUDE_SKILL_DIR}/scripts/check_docs.ts --target . --min-coverage 80
```
  ...
  - repo: local
    hooks:
      - id: doc-coverage
        ...
        entry: bun run .claude/skills/doc-coverage/scripts/check_docs.ts --git-staged --min-coverage 80
        language: system
  ...
  doc-coverage:
    ...
    steps:
      ...
      - name: Check documentation coverage
        run: |
          bun run .claude/skills/doc-coverage/scripts/check_docs.ts \
            --target . \
            --min-coverage 80 \
            --format json \
...
|--------|---------|------------|
| `scripts/check_docs.ts` | Main documentation coverage scanner | 0=pass, 10=below threshold, 1=error |

---

## Extension Points

1. **New languages**: Add parser in `check_docs.ts` for additional languages

...
| `ADR file created or modified` | Auto-triggered via detect_adr_changes.ts  |
...
```text
# Manual triggers:
/adr-review decisions/ADR-005-api-versioning.md
"review this ADR"
"validate ADR-005"
```
...
| ---------- | -------------- | ---------------------- |
| `ADR-*.md` | `decisions/`   | create, update, delete |

**Detection**: `${CLAUDE_SKILL_DIR}/scripts/detect_adr_changes.ts`
...
- ANY agent modifies `decisions/ADR-*.md`
...
| Script                  | Purpose                                  |
| ----------------------- | ---------------------------------------- |
| `detect_adr_changes.ts` | Detect ADR file changes for auto-trigger |
...
bun run ${CLAUDE_SKILL_DIR}/scripts/detect_adr_changes.ts --since-commit abc123

# Include untracked ADR files
bun run ${CLAUDE_SKILL_DIR}/scripts/detect_adr_changes.ts --include-untracked
```
...
- [ ] Debate log exists as Brain memory note `CRIT-{NNN}-debate-log` in `critique/` folder
...
| Manual ADR monitoring                  | Error-prone              | Use detect_adr_changes.ts    |

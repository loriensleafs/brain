...
|--------|----------|-------|
| `scripts/detect_infrastructure.ts` | Bun TypeScript | Cross-platform |

## Usage

```bash
# Analyze staged files
bun run ${CLAUDE_SKILL_DIR}/scripts/detect_infrastructure.ts --use-git-staged

# Analyze specific files
bun run ${CLAUDE_SKILL_DIR}/scripts/detect_infrastructure.ts .github/workflows/ci.yml src/auth/login.cs
```
...
Run security agent before implementation:
  Agent(subagent_type="security", prompt="Review infrastructure changes")
```
...
bun run .claude/skills/security-detection/scripts/detect_infrastructure.ts --use-git-staged
```
...
- name: Check security-critical files
  run: bun run .claude/skills/security-detection/scripts/detect_infrastructure.ts --use-git-staged
```

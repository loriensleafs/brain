...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/select_depth.ts --budget 100000 --impact medium --reversibility moderate
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/calculate_tco.ts \
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/score_decision.ts \
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/calculate_tco.ts --help
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/score_decision.ts --help
...
### check_reassessment_triggers.ts

Detect assumption drift, recommend re-evaluation.

```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/check_reassessment_triggers.ts --help
...
```bash
bun run ${CLAUDE_SKILL_DIR}/scripts/score_vendor.ts --help
...
bun run ${CLAUDE_SKILL_DIR}/scripts/check_reassessment_triggers.ts \

...
allowed-tools:
  ...
  - Write
  - Edit
agents: [planner, analyst, architect, critic]
metadata:
  version: 4.2.0
...
bun scripts/triage_skill_request.ts "help me debug this error"

# JSON output for automation
bun scripts/triage_skill_request.ts "create a skill for payments" --json

# Examples:
bun scripts/triage_skill_request.ts "TypeError: Cannot read property 'map'"
# -> USE_EXISTING: Recommends ErrorExplainer (92%)

bun scripts/triage_skill_request.ts "create a skill for code review"
# -> CLARIFY: CodeReview skill exists (85%), create anyway?

bun scripts/triage_skill_request.ts "help me with API and auth and testing"
...
bun scripts/discover_skills.ts
...
bun scripts/quick_validate.ts ~/.claude/skills/my-skill/

# Full structural validation
bun scripts/validate_skill.ts ~/.claude/skills/my-skill/

# Package for distribution
bun scripts/package_skill.ts ~/.claude/skills/my-skill/ ./dist
```
...
└── scripts/                    # Automation scripts (optional)
    ├── validate.ts             # Validation/verification
    ├── generate.ts             # Artifact generation
...
**Script Requirements:**
- Bun TypeScript with no external dependencies (use Bun/Node stdlib)
- `Result` type pattern for structured returns
...
hooks:
  PreToolUse:
    - matcher: "Bash(bun:scripts/*)"
      hooks:
        - type: command
          command: "bun scripts/quick_validate.ts . 2>/dev/null || true"
          once: true
```
...
- [ ] `bun scripts/quick_validate.ts` passes
- [ ] `bun scripts/check_docs_safety.ts` passes
...
### v4.2.0 (Current)
- **Python to TypeScript migration** - All scripts converted from Python to Bun TypeScript
- **Brain MCP integration** - Replaced Serena/Forgetful memory operations with Brain MCP equivalents
- **Agent field added** - Added `agents` field to frontmatter
- **Script template updated** - Template converted to TypeScript with Bun patterns

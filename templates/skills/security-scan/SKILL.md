---
name: security-scan
description: Scan code content for CWE-22 (path traversal) and CWE-78 (command injection) vulnerabilities before PR submission. Lightweight pattern-based detection for Python, TypeScript, Bash, and C# files. Use when preparing code for review or as a pre-commit gate.
license: MIT
agents: [security, qa, implementer]
...
|----------------|-----------|
| `scan for vulnerabilities` | scan_vulnerabilities.ts on staged/specified files |
| `check for path traversal` | scan_vulnerabilities.ts with CWE-22 focus |
| `check for command injection` | scan_vulnerabilities.ts with CWE-78 focus |
| `pre-PR security scan` | scan_vulnerabilities.ts on staged files |
| `run security scan` | scan_vulnerabilities.ts with full scan |
...
| Script | Purpose |
|--------|---------|
| `scripts/scan_vulnerabilities.ts` | Main scanner for CWE-22 and CWE-78 patterns |
...
```bash
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts --git-staged
```

### Scan Specific Files

```bash
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts path/to/file.py another/script.ps1
```

### Scan Directory

```bash
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts --directory src/
```

### JSON Output (CI Integration)

```bash
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts --git-staged --format json
```
...
```bash
# Path traversal only
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts --cwe 22 --git-staged

# Command injection only
bun run .claude/skills/security-scan/scripts/scan_vulnerabilities.ts --cwe 78 --git-staged
```
...
- Analysis: Closed PR reviewer patterns.
...
|-----------|------------|
| New CWE patterns | Add to `PATTERNS` dict in scan_vulnerabilities.ts |
